...
def graph(graph_vals, matrix):
	...
	Parameters
	----------
	graph_vals - list containing: 1. number of spaces in the grid
	(difficulty) and 2. Average of time taken to solve grid three times.
	matrix - the product of n_rows and n_cols for the grid.

	Output:
	-------
	Plots time taken to solve vs difficulty for each graph.
	A plot for each dimension of grid is produced.

	'''
	#sorting the list in ascending order based on the first element in the list:
	...
	#separating the nested list into one for time and one for difficulty to be plotted against eachother.
	...
	# Specifying which dimension of grid is being shown.
	if matrix == 4:
